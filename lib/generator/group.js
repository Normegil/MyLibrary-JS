...
function generate(name, numberOfUsers, hasAccess) {
  log.info(
    {name: name, numberOfUsers: numberOfUsers},
...
function clean(callback) {
  ...
  Group.find(function onFound(err, groups) {
    if (err) {
      return callback(err);
    }
    async.each(
