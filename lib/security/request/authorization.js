...
function checkRights(options, callback){
	...
	log.trace({method: method, resource: resource, userRights: user.access}, 'Check user access to ' + user.pseudo);
	var userAccess = checkAccessRights(user.access, resource, method);
	if(userAccess){
		log.trace({method: method, resource: resource, userPseudo: user.pseudo}, 'Access granted: ' + user.pseudo);
		...
		Group.find({users: user}, function onGroupLoad(err, groups){
			...
			if (!h.exist(groups)) {
				...
				log.trace({method: method, resource: resource, groups: groups}, 'Check groups access of user: ' + user.pseudo);
