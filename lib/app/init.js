...
function init(baseRestPath, callback) {
  ...
  async.series([
    ...
    function generatebaseUserAndGroups(callback) {
...
function keysGeneration(callback) {
  ...
  initLog.debug('Trying to find key: ' + keyOptions.name);
  Key.findOne({name: keyOptions.name}, function onLoad(err, key) {
    if (err) {
      initLog.error(
        {keyOptions: keyOptions},
        'Error while trying to find key: ' + keyOptions.name);
      return callback(err);
    }
    if (h.exist(key)) {
      secLog.debug(
        {keyOptions: keyOptions},
        'Security key existing - Skipping generation');
      ...
      security.generateKey(keyOptions, function onGenerated(err, key) {
        ...
        secLog.debug(
          {keyOptions: keyOptions},
          'Saving key to database: ' + keyOptions.name);
        key.save(function onSave(err) {
          if (err) {
            initLog.error(
              {keyOptions: keyOptions},
              'Error while saving key: ' + keyOptions.name);
            return callback(err);
          }
          secLog.debug(
            {keyOptions: keyOptions},
            'Key saved database: ' + config.security.token.key.name);
          return callback(null, true);
        });
      });
    }
...
function userGroupInitialisation(baseRestPath, callback) {
  ...
  User.findOne({pseudo: guestPseudo}, function onLoad(error, user) {
    if (error) {
      initLog.error(
        {pseudo: guestPseudo},
        'Error when trying to find guest user');
...
function generateGuest(guestPseudo, baseRestPath, callback) {
  initLog.debug(
    {pseudo: guestPseudo, basePath: baseRestPath},
    'Creating guest user');
  var guest = new User();
  initLog.debug('Generating hashed password for guest user');
  security.getPasswordHash(
    guestPseudo,
    function assignHashAndAccessRights(err, hash) {
      if (err) {
...
function generateAccess(user, baseRestPath, callback) {
  ...
  user.access = u.map(routesList, function addAccessRight(route) {
    return {
  ...
  initLog.debug(
    {accessRights: user.access},
    'Guest user\'s access rights generated');
