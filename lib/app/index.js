...
function launch(port) {
  ...
  init(BASE_REST_PATH, function launchServer(err) {
    ...
    setupPublicRessources();

    loadIgnoredPaths(function onLoad(err, whiteList) {
      if (err) {
        initLog.fatal(
          {error: err},
          'Error while parsing file defining URL paths ignored by security');
        throw err;
      }
...
function loadIgnoredPaths(callback) {
  ...
  csv.parse(
    __dirname + '/SecurityIgnoredPath.csv',
    function parseIgnoredPaths(err, securityIgnoredPaths) {
      if (err) {
        return callback(err);
      }
      var whiteList = u.map(securityIgnoredPaths, parseIgnoredPathLine);
      secLog.debug(
        {SecurityIgnoredPath: whiteList},
        'finish loading URL paths ignored by security');
...
function setupPublicRessources(app, baseRestPath, router) {
  initLog.debug('Set static paths for REST services and public ressources');
  app.use(baseRestPath, router);
