...
function getAll(request, response){
  log.info({req: request, res: response}, 'Receive GET all request for BookSerie');
  ...
  getAllQuery.exec(function onSelect(err, books){
    if(err) {
      log.error({query: getAllQuery, err: err}, 'Error when trying to find requested BookSerie');
      ...
      BookSerie.count(function onCount(err, count){
        if(err) {
          log.error({err: err}, 'Counting total number of BookSeries has failed');
          rest.error.handle(response, 50000, err);
        }  else {
...
function get(request, response){
	log.info({req: request, res: response}, 'Receive GET request for BookSerie: ' + request.params.id);
  BookSerie.findById(request.params.id, function(err, book){
    if(err){
      ...
      rest.error.handle(response, 50000, err);
			...
			rest.error.handle(response, 40400, new Error('BookSerie ' + request.params.id + ' not found'));
    }else{
			book = book.toObject();
...
function create(request, response){
	log.info({req: request, res: response}, 'Receive creation request for BookSerie');
  var book = new BookSerie();
  book = request.body;
	log.debug({bookSerie: book}, 'New BookSerie created');
  book.save(function(error){
    if(error){
			return rest.error.handle(response, 50000, error);
		}
    var location = request.protocol + '://' + request.get('Host') + request.originalUrl + '/' + book._id;
...
function update(request, response){
  log.info({req: request, res: response}, 'Updating BookSerie ' + request.params.id);
  BookSerie.findById(request.params.id, function(err, bookSerie){
    if(err){
			log.error({err: err}, 'Error while loading BookSerie: ' + request.params.id);
			...
			log.debug({bookSerie: bookSerie}, 'Finish updating BookSerie: ' + request.params.id);
	    bookSerie.save(function(error){
	      if(error) {
					log.error({err: err}, 'Error while saving BookSerie: ' + request.params.id);
					return error.handle(response, 50000, error);
				}
				log.debug({err: err}, 'BookSerie updated and saved: ' + request.params.id);
	      response
	        .status(rest.status.ok)
	        .send();
	    });
...
function remove(request, response){
	log.info({req: request, res: response}, 'Deleting BookSerie ' + request.params.id);
  BookSerie.remove({ _id: request.params.id }, function(err){
		if(err) {
			log.error({err: err, idBooksSerie: request.params.id}, 'Error when deleting BookSerie: ' + request.params.id);
			return rest.error.handle(response, 50000, err);
		}
